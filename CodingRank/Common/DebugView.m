...
- (void)nextView {
    ...
    if ([_container.subviews count] == 0)
        return;
    NSInteger index = [_container.subviews indexOfObject:_view];
    index++;
...
- (void)drawRect:(CGRect)rect {
    ...
    //显示当前视图的坐标
    CGContextSetRGBFillColor(context, 0.0, 0.0, 0.0, 1.0);
    NSString *viewName = [NSString stringWithFormat:@"%@, views:%d velocity:%d", NSStringFromClass([_view class]), (int )[_container.subviews count] - 1,_velocity];
